...
%   Negative Log Likelihood Function - Optimistic Alpha (Positivity Bias) 

function [nll_alpha_opt] = RL_unbiased_nll_alpha_opt(choices, rewards,T, alpha_opt, beta) 
%%  Cleanup 
...
logptrial_alpha_opt = NaN(T,1); % placeholder vector of NaNs to record log likelihood per trial
...
%if prediction_error > 0 
    Q(states(t), choices(t)) = Q(states(t), choices(t)) + alpha_opt*prediction_error*(prediction_error>0);
%else
...
logptrial_alpha_opt(t) = log(choice_probabilities(choices(t)));  % calculating log likelihood for each trial 

end % end the trial loop

loglikesum_alpha_opt = sum(logptrial_alpha_opt);    % summing log likelihoods across trials

nll_alpha_opt = (-1)*(loglikesum_alpha_opt);        % calculating negative log likelihood
